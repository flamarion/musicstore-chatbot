...
- ✅ 9 tools bound to LLM: purchase history, recommendations, inventory, artist lookup, albums by artist, genre catalog, genre browse, top sellers, store reference
...
- ✅ **Full Docker Compose deployment** — LangGraph agent server (Postgres/Redis) + LangChain agent-chat-ui on the `ai-stack` network; runs on a laptop with **no bundled model** (the agent uses a remote `LLM_ENDPOINT` or hosted Claude from `.env`); one `docker compose up --build`
...
## What's Done
- **LLM-driven agent**: LangChain `create_agent` (prebuilt ReAct loop) over `ChatOpenAI` (any OpenAI-compatible endpoint) or `ChatAnthropic`, with cross-cutting concerns as middleware — no hand-rolled `StateGraph`
- **9 tools**: `purchase_history_tool`, `recommendation_tool`, `inventory_tool`, `artist_lookup_tool`, `albums_by_artist_tool`, `genre_catalog_tool`, `browse_genre_tool`, `top_sellers_tool`, `store_reference_tool`; each tool object carries a category tag (`catalog`/`account`/`reference`) + `reads_pii` metadata so its `tool` run is filterable/groupable in LangSmith
...
- **Deployment**: `docker compose up --build` brings up the LangGraph agent server (built from the generated `Dockerfile`, `app.py:make_graph`) + Postgres + Redis + `agent-chat-ui` (proxy mode) on the external `ai-stack` network — no bundled model, so it runs on a laptop; the containerized agent reads the model backend (`LLM_ENDPOINT` to a remote host, or `ANTHROPIC_API_KEY`) from `.env`; needs `LANGSMITH_API_KEY` (server license)
...
- **Topic guardrails**: `TopicGuardMiddleware.before_model` runs a lightweight keyword pre-filter (`_is_off_topic`) and `jump_to="end"` on off-topic messages — no LLM call, no tool execution. Deliberately narrow: it blocks only on a curated off-topic keyword hit and lets everything else through (false positives are worse than false negatives); the system prompt's "stay in scope" rule is the real backstop. Stateless — split from the profanity guard so each middleware does one thing.
...
## Recent Changes
- 2026-07-08: **Compose runs model-free** — removed the `llama` (GPU llama.cpp) service from `docker-compose.yml` and dropped the compose `LLM_ENDPOINT`/`LLM_PROVIDER` overrides. The stack (agent server + Postgres + Redis + agent-chat-ui) now runs on a plain laptop and the containerized agent reads its model backend from `.env` — a remote OpenAI-compatible `LLM_ENDPOINT` or `ANTHROPIC_API_KEY`, same as the standalone CLI. Docs (README stack section, CLAUDE, `.env.example`) synced; `llama.env*` no longer used by the default stack.
- 2026-07-08: **`albums_by_artist_tool`** — new catalog lookup that lists a named artist's albums (fuzzy `LIKE` match, annotated with units sold, grouped per matching artist). Closes the gap where "which Nirvana albums do you have?" had no tool to answer it — `artist_lookup_tool` only returned matching artist *names* and `browse_genre_tool` only worked by genre. Wired into the tool list + system prompt; verified end-to-end (the Nirvana conversation now lists both albums on the first ask).
- 2026-07-08: **Per-tool LangSmith trace tagging** — each tool object gets a category tag (`catalog`/`account`/`reference`) + `{tool_category, reads_pii}` metadata (`_tag_tool_for_tracing`), which propagate into its `run_type="tool"` run. Lets the LangSmith Runs view filter/group tool calls (e.g. `tag=pii` for personal-data lookups) without drilling into individual traces; `reads_pii` derives from `_PII_CONSENT_PURPOSE` so it can't drift from the consent gate.
- 2026-07-08: **Static-type cleanup (`app.py`)** — resolved all pyright/Pylance errors (0 remaining): annotated the middleware stack as `list[AgentMiddleware[Any, Any, Any]]` (the invariant `StateT` across heterogeneous middleware was breaking `create_agent`'s overloads), typed the `__main__` invoke `config` as `RunnableConfig`, and typed `build_llm`'s `kwargs` as `dict[str, Any]` (folding `model` into it to sidestep ChatAnthropic's pydantic-alias param). Behavior-preserving.
- 2026-07-08: **Simplification pass** — collapsed the topic classifier from a three-tier scheme into a single `_is_off_topic()` check: the on-topic keyword set and short allowlist were provably inert (both fell through to the same "on-topic" default), so ~55 lines (including 10 hardcoded customer names) were removed with no behavior change. Also keyed the HITL consent purpose off a `_PII_CONSENT_PURPOSE` map (single source of truth with `_PII_CONSENT_TOOLS`) and dropped a derivable `(count, list)` tuple from `_lookup_customer_by_exact_email`.
